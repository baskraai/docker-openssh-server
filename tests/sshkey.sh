...
export SSH_KEY
docker run -d --rm -p 2222:22 --name openssh-server -e NAME=test -e USERNAME=test -e PASSWORD=password -e SSH_AUTHKEY="$(cat "$HOME"/.ssh/id_ecdsa.pub)" openssh-serve > /dev/nullr
